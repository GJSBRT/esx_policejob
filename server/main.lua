...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			-- can the player carry the said amount of x item?
				...
				PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Politie fouiller log [ITEMS]", description = "**Agent** "..sourceXPlayer.name.." **heeft** "..amount.." **x** "..sourceItem.label.. "** afgepakt van **"..targetXPlayer.name.."**\nSteamnaam van agent : **"..GetPlayerName(source).."**\nSteamnaam van verdachte : **"..GetPlayerName(target).."", footer = { text = " © 𝘿𝙧𝙪𝙜𝙖𝙣𝙤𝙫#6843 "}, color=65535}}}),  { ['Content-Type'] = 'application/json' })
	...
	elseif itemType == 'item_account' then
		...
		if targetAccount.money >= amount then
			...
			PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Politie fouiller log [ZWARTGELD]", description = "**Agent** "..sourceXPlayer.name.." **heeft** "..amount.." **euro zwartgeld afgepakt van **"..targetXPlayer.name.."**\nSteamnaam van agent : **"..GetPlayerName(source).."**\nSteamnaam van verdachte : **"..GetPlayerName(target).."", footer = { text = " © 𝘿𝙧𝙪𝙜𝙖𝙣𝙤𝙫#6843 "}, color=0}}}),  { ['Content-Type'] = 'application/json' })
		else
			sourceXPlayer.showNotification(_U('quantity_invalid'))
		end
	...
	elseif itemType == 'item_weapon' then
		...
		if targetXPlayer.hasWeapon(itemName) then
			...
			PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "Politie fouiller log [WAPENS]", description = "**Agent** "..sourceXPlayer.name.." **heeft een** "..ESX.GetWeaponLabel(itemName).. "** afgepakt van **"..targetXPlayer.name.."** met **"..amount.."** kogels\nSteamnaam van agent : **"..GetPlayerName(source).."**\nSteamnaam van verdachte : **"..GetPlayerName(target).."", footer = { text = " © 𝘿𝙧𝙪𝙜𝙖𝙣𝙤𝙫#6843 "}, color=16711680}}}),  { ['Content-Type'] = 'application/json' })
		else
			sourceXPlayer.showNotification(_U('quantity_invalid'))
		end
	end
...
AddEventHandler('esx_policejob:giveWeapon', function(weapon, ammo, token)
	local _source = source
